...
<%    
    ...
    if(email != null || session.getAttribute(email) != null){  
            try{ 
                ...
                ResultSet rs2 = st2.executeQuery("select * from dislike where user_email='"+email+"' AND code='"+vcode+"'");
                
                if(rs2.next()){                
                    Statement st1 = cn.createStatement(); 
                    if(st1.executeUpdate("delete from dislike where code='"+vcode+"'")>0){
                       out.print("deleted");
                    }
                    else{
                    ...
                    ResultSet rs1 = st.executeQuery("select * from vlike where user_email='"+email+"' AND code='"+vcode+"'");
                    if(rs1.next()){
                        if(st.executeUpdate("delete from vlike where code='"+vcode+"' AND user_email='"+email+"'")>0){
                            out.print("deleted");
                        }
                    }                    
